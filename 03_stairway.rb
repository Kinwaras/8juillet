...
num_de = x
    ...
    if num_de == 5 || num_de == 6
      @etage_debut +=1
      puts "Vous avez avance  et votre etage actuel est : #{@etage_debut} et vous venez de l'etage : #{@etage_debut - 1}"
    end
    if num_de == 1
      @etage_debut -=1
      if @etage_debut < 0
...
def manuel(nbr_partie)
  ...
  @etage_debut = 0
  for i in (1..nbr_partie)
    puts "entrez manuellement le numéro affiché sur le dé"
    print "> "
...
def partie(nbr_partie)
  ...
  while user_input !=3
    ...
    puts "tapez '2' pour jouer manuellement"
    puts ""
